// Shows mobile users a swipe hint animation on first visit.
// Only runs on mobile devices, once per session.
export function useMobileSwipeHint(projectCount: Ref<number>) {
  const { isMobile } = useDevice()
  const hasShownMobileHint = ref(false)

  function setup() {
    if (!isMobile || hasShownMobileHint.value || projectCount.value === 0)
      return
